...
export default function WorkoutSession({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  async function logSet() {
    ...
    const set: WorkoutSetLog = {
      id: id(), sessionId, exerciseId: next.exerciseId, sequence: session.sets.filter(s => s.exerciseId === next.exerciseId).length + 1,
      loadKg, reps, rpe, setType: next.setType, recordedAt: now
    };
    const symptom: SymptomObservation | undefined = symptomSeverity > 0 ? {
      id: id(), sessionId, exerciseId: next.exerciseId, severity: symptomSeverity,
  ...
  async function finishSession() {
    ...
    const recommendation: RecommendationSnapshot = {
      id: id(), ruleVersion: decision.ruleVersion, createdAt: new Date().toISOString(),
