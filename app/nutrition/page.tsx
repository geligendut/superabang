...
export default function NutritionPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!auth.session?.user) {
        setRows([]);
        setMessage('Sign in to log and view nutrition.');
        return;
      }
      ...
      setMessage(e instanceof Error ? e.message : 'Unable to load nutrition.');
    }
  }, []);
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      setDescription(''); setPortionNote('');
      setEnergy(''); setProtein(''); setCarbs(''); setFat('');
      setBasis('UNKNOWN'); setConfidence(''); setCoverage('PARTIAL'); setMealType('OTHER');
      setMessage('Meal saved.');
      await load();
    } catch (err) {
      setMessage(err instanceof Error ? err.message : 'Unable to save meal.');
    } finally {
      setSaving(false);
    }
  }
  ...
  return <main>
    ...
    <h1>Nutrition</h1>
    <p className="muted">
      Manual meal observations with explicit uncertainty. Partial meal logging is preserved and never promoted to a complete daily intake total.
    </p>
      ...
      <div className="form-grid">
        <label>Meal type
          <select value={mealType} onChange={e => setMealType(e.target.value as MealType)}>
            ...
            <option value="DINNER">Dinner</option><option value="SNACK">Snack</option><option value="OTHER">Other</option>
          </select>
        </label>
        <label>Coverage
          <select value={coverage} onChange={e => setCoverage(e.target.value as MealCoverage)}>
            <option value="PARTIAL">Partial / may be incomplete</option>
            <option value="COMPLETE">Complete meal description</option>
          </select>
        </label>
      </div>
      ...
      <div className="form-grid">
        <label>Nutrition basis
          <select value={basis} onChange={e => {
            const next = e.target.value as NutritionBasis;
            setBasis(next);
            if (next === 'UNKNOWN') setConfidence('');
          }}>
            <option value="UNKNOWN">Unknown / no nutrition values</option>
        ...
        <label>Confidence
          <select value={confidence} disabled={basis === 'UNKNOWN'} onChange={e => setConfidence(e.target.value as NutritionConfidence | '')}>
    ...
    {rows.map(row => <div className="card" key={row.id}>
      ...
      <div className="muted">
        Energy {fmt(row.energyKcal, 'kcal')} · Protein {fmt(row.proteinG, 'g')} · Carbs {fmt(row.carbsG, 'g')} · Fat {fmt(row.fatG, 'g')}
      </div>
    </div>)}
  </main>;
}
